...
J = 1 / m * sum(sum((-Y .* log(a3) - (1 - Y) .* log(1 - a3)))) + Jr;

% step 2: error on 3th layer
delta3 = a3 - Y;

% step 3: error on 2th layer
delta2 = (delta3*Theta2).*sigmoidGradient([ones(size(z2, 1), 1) z2]);
delta2 = delta2(:, 2:end);

% step 4: accumulative gradient (gradient descent)
delta_1 = (delta2'*a1);
delta_2 = (delta3'*a2);

% step 5: Obtain the regularized gradient
