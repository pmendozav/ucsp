...
vec_t<vec_s> _ViterbiV2::Predict(vec_t<vec_s> sentences)
{
	vec_t<vec_s>
		result;
	int i = 0;
...
vec_s _ViterbiV2::Predict(vec_s words)
{
	vec_s
		result;
	Map3d_dssf
	...
	for (int k = 1; k <= n; k++)
	{
		...
		if (it == e.end())
		{
			if (IsNumeric(xk))
				it = e.find("_NUMERIC_");
			else if (IsCapitalized(xk))
				it = e.find("_CAPITALIZED_");
			else if (IsLastCapital(xk))
				it = e.find("_LASTCAPITAL_");
			else
				it = e.find("_RARE_");
		}
		xk = it->first;
...
string _ViterbiV2::Predict(string sentence)
{
	vec_s
		result,
		words;
...
void _ViterbiV2::Train(string ifname)
{
	Calc_E(ifname);
	Calc_Q(ifname);
}

void _ViterbiV2::Calc_E(string ifname)
{
	Map2d::iterator
		it2d;
	Map1d::iterator
...
void _ViterbiV2::Calc_Q(string ifname)
{
	Map3d::iterator
		it3d;
	Map2d::iterator
